	...
	return {
		...
		setAuto: function(callback) {
			...
			this.modeAutoInterval = setInterval(function() {
				var fakeCurrentTemperature = 21;
				console.log("AUTO Adjust temp from/to", fakeCurrentTemperature, this.targetTemperature);
				if(this.targetTemperature >= fakeCurrentTemperature + 0.25) {
					//No heat
					this.gpioPosState = false;
					this.gpioNegState = true;
					this.gpioApply();
					console.log("Hretter than: no heat");
				} else if(this.targetTemperature <= fakeCurrentTemperature - 0.25) {
					//Heat
					this.gpioPosState = false;
					this.gpioNegState = false;
					this.gpioApply();
					console.log("Less than: heat");
				} else {
