	...
	return {
		...
		setAuto: function(callback) {
			...
			this.modeAutoInterval = setInterval(function() {
				...
				console.log("AUTO Adjust temp from/to", sensordata.temperature, this.targetTemperature);
				if(this.targetTemperature >= sensordata.temperature + 0.25) {
					//No heat
					this.gpioPosState = false;
					this.gpioNegState = true;
					this.gpioApply();
					console.log("Hretter than: no heat");
				} else if(this.targetTemperature <= sensordata.temperature - 0.25) {
					//Heat
					this.gpioPosState = false;
					this.gpioNegState = false;
					this.gpioApply();
